...
#include "esp_log.h"
#include "nvs_flash.h"
#include "../mdns/mdns/include/mdns.h"
#include "lwip/err.h"
#include "lwip/sys.h"
    ...
    while(1)
    {
        ...
        while (1) {
            ...
            // Data received
                ...
                int gpio_level=rx_buffer[0]-'0';
                if (gpio_level == 0 || gpio_level == 1) {
                    gpio_set_level(GPIO_OUTPUT_IO, gpio_level);
                    ESP_LOGI(TAG, "Set GPIO level to: %d", gpio_level);
                } else {
                    ESP_LOGE(TAG, "Invalid GPIO level received: %d", gpio_level);
                }                
...
void start_mdns_service(void)
{
    ...
    ESP_ERROR_CHECK(mdns_hostname_set("esp32-sacrieriu"));
    ESP_LOGI(TAG, "mDNS hostname set to esp32-sacrieriu.local");
    ESP_ERROR_CHECK(mdns_instance_name_set("ESP32 IoT Device"));
    ESP_ERROR_CHECK(mdns_service_add("ESP32_Service", "_http", "_tcp", 80, NULL, 0));
}
...
void mdns_query_task(void* pvParameters)
{
    while (1) {
        ...
        if (err) {
            ...
            ESP_LOGI(TAG, "Discovered ESP32 devices in network:");
            mdns_result_t *r = results;
            while (r) {
                ESP_LOGI(TAG, "Hostname: %s, IP: " IPSTR, r->hostname, IP2STR(&(r->addr->addr.u_addr.ip4)));
                r = r->next;
            }
...
void mdns_query_task(void *pvParameters) {
    ...
    while (1) {
        ESP_LOGI(TAG, "Querying all mDNS services...");
        esp_err_t err = mdns_query_ptr("_ni-logos", "_tcp", 3000, 10, &results);
        ...
        if (err != ESP_OK) {
            ...
            ESP_LOGI(TAG, "Discovered mDNS services:");
            mdns_result_t *r = results;
            while (r) {
