...
void GPIO_ODD_IRQHandler(void)
{
  ...
  sl_bt_gatt_server_write_attribute_value(29,
                                          0,
                                          sizeof(button_state),
                                          &button_state);

  if (button_notification_enabled) {
      sl_bt_gatt_server_notify_all(29,
                                  sizeof(button_state),
                                  &button_state);
    }
}
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_sm_passkey_display_id:
      app_log("Parola este: %lu",evt->data.evt_sm_passkey_display.passkey);
      break;
      
    case sl_bt_evt_sm_bonding_id:
      app_log("Conexiune reusita!/r/n");
      break;
    case sl_bt_evt_sm_bonding_failed_id:
         app_log("Eroare la conexiune! %lu/r/n",evt->data.evt_sm_bonding_failed.reason);
         break;
      
    // -------------------------------
    // This event indicates that a connection was closed.
    case sl_bt_evt_connection_closed_id:
      app_log("Connection closed. Bond deleted;\r\n");
    ...
    case sl_bt_evt_gatt_server_attribute_value_id:
      if (27 == evt->data.evt_gatt_server_characteristic_status.characteristic) {
        sl_bt_gatt_server_read_attribute_value(27,
                                            0,
                                            sizeof(recv_val),
                                            &recv_len,
                                            &recv_val);
        ...
        led_state = recv_val;
        app_log("Trebuie sa am valoarea %d \r\n",led_state);
        if (led_state) {
    ...
    case sl_bt_evt_gatt_server_characteristic_status_id:
      if (29 == evt->data.evt_gatt_server_characteristic_status.characteristic) {
          if (evt->data.evt_gatt_server_characteristic_status.client_config_flags & sl_bt_gatt_notification) {
              app_log("Notificari activate pentru BUTTON");
              button_notification_enabled = true;
          } else {
              app_log("Notificari dezactivate pentru BUTTON");
